...
import { createTitle } from './tab01_DOM.js';
...
function preparePrintOverview() {
    ...
    const endDate = el("li", { html: `<strong data-i18n="print.end-date">${t('print.end-date')}</strong> <span>${$('#eindDatum').textContent || "-"}</span>` });
...
export function printData() {
    ...
    // Hide all elements except the ones we want to print
    const tab01Content = document.querySelectorAll('#tab01 > *:not(.table-generator)');
    const tableGeneratorContent = document.querySelectorAll('.table-generator > *:not(#annualReportOutput):not(#leningOverzicht)');
    
    const originalStyles = new Map();
    
    // Hide everything in tab01 except table-generator
    tab01Content.forEach(el => {
        if (!el.classList.contains('table-generator')) {
            originalStyles.set(el, el.style.display);
            el.style.display = 'none';
        }
    });
    
    // Hide everything in table-generator except the output we want
    tableGeneratorContent.forEach(el => {
        originalStyles.set(el, el.style.display);
        el.style.display = 'none';
    });
    ...
    // Restore all styles after print
    setTimeout(() => {
...
function createCheckboxes() {
    return el('div', { class: 'cb-container' }, [
        ...
        el('div', { class: 'columns-container no-print' }, [
...
function createButtons() {
    return el("div", { class: "button-group" }, [
